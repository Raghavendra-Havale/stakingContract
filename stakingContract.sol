...
//Write a smart contract for staking 
//staking is for 6 months
//user can stake anytime
//unstaking can happen anytime
//user can stake any no of times
//based on time remaining, slashing(deduction due to early withdrawal) is calculated
//max slash %age is 10%
//min is 0.5%
//200%APR, min APR 1%, APR will drop with increase in TVL. decay rate can be customized. 
//if total TVL is more than or equal to 10M, APR will be flat 25%
...
contract Staking {
    ...
    function checkReward(uint256 stakeId) public view returns (uint256) {
        ...
        uint256 reward = (stakeInfo.amount * (block.timestamp - stakeInfo.stakeInTimestamp) * currentAPR) / (60 * 60 * 24 * 365 * 100);
    ...
    function claimReward(uint256 stakeId) public {
        ...
        StakeInfo storage stakeInfo = userStakes[msg.sender][stakeId];
        stakeInfo.stakeInTimestamp = block.timestamp; // Reset timestamp after claiming reward
